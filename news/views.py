        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                if news_dict['pub_date'] and news_dict['pub_date'] != '未知日期':
                    try:
                        # 将日期转换为字符串
                        date_str = str(news_dict['pub_date'])
                        # 日期格式是"2025-4"，添加"月"字
                        if '-' in date_str:
                            year, month = date_str.split('-')
                            news_dict['pub_date'] = f"{year}年{month}月"
                        else:
                            news_dict['pub_date'] = date_str
                    except Exception as e:
                        print("日期处理错误:", str(e))  # 调试信息
                        news_dict['pub_date'] = str(news_dict['pub_date'])
                else:
                    news_dict['pub_date'] = "未知日期"
...
def news_detail(request, news_id, model_type):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # 处理日期格式
            if news['pub_date'] and news['pub_date'] != '未知日期':
                try:
                    # 将日期转换为字符串
                    date_str = str(news['pub_date'])
                    # 日期格式是"2025-4"，添加"年"和"月"字
                    if '-' in date_str:
                        year, month = date_str.split('-')
                        news['pub_date'] = f"{year}年{month}月"
                    else:
